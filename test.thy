...
proof (induct t arbitrary: p)
 ...
 using branch_in_tree local.Cons nNode.hyps sub_tree_set by presburger
    ...
    moreover
    have "foldr max (map nHeight (t # ts')) 0 \<ge> nHeight sub_tree"
...
proof -
  from assms(2)
  have p_len: "length p = nHeight t"
    unfolding nLongest.simps by auto
  with assms(3)
  have len_p': "length p' = nHeight t - 1"
    by auto

  from assms(1)
  have t_height: "nHeight t = Suc (foldr max (map nHeight (x#xs)) 0)"
    by auto

  from len_p' t_height
  have len_p'_eq: "length p' = foldr max (map nHeight (x#xs)) 0"
    by simp

  from assms(1,2,3)
  obtain t'' where t''_def: "t'' \<in> set (x#xs)" "p' \<in> set (nPaths t'')"
    by auto

  have "nHeight t'' = length p'"
  proof -
    from t''_def(1)
    have "foldr max (map nHeight (x#xs)) 0 \<ge> nHeight t''"
      using subtree_height by fastforce
    moreover from t''_def(2)
    have "nHeight t'' \<ge> length p'"
...
proof -
  ...
  from assms(1)
  have "nHeight t = Suc (foldr max (map nHeight (x#xs)) 0)"
    by simp
  then have "nHeight t = Suc (nHeight t')"
    using foldr_max_eq2[OF assms(3,4)] by simp
  moreover from p_def
  obtain p' where "p = n # p'"
    using assms by auto
  
  ultimately
  have len_p': "length p' = nHeight t'"
    using assms by auto

  from assms(3-4)
  have "\<forall>t'' \<in> set (x#xs). nHeight t'' \<le> nHeight t'"
  ...
  proof -
    ...
    ultimately
    show ?thesis using len_p' by linarith
  qed

  with t'_def(2)
  have "p' \<in> nLongest t''"
    unfolding nLongest.simps by auto
...
next
  ...
  from Suc.IH[OF check_p]
  have "nCheck n d p" .
  with p_in_ts height_prop
  show
  ?case using t_def by auto
qed
