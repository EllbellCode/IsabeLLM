...
proof (induction rule: traces.induct)
  case (honest_base)
  then show ?case
    by (simp add: b1 nCheck_add_h) (*SAFE*)
next
  case (dishonest_base)
  by simp
  show ?case
by (metis Suc_pred add_diff_cancel_left' b2 base_check count_dhonest_false_ind count_dhonest_true_ind count_false_base count_true_base diff_zero list.sel(1) n_check_weaken_distance_Node select_convs(1,2) zero_less_diff) (*SAFE*)
next
  case (honest_induct t)
  let ?c1 = "count True t"
  let ?c2 = "count False t"
  have "hd (\<lparr>Honest = True, State = hadd (State (hd t))\<rparr> # t) = \<lparr>Honest = True, State = hadd (State (hd t))\<rparr>" by simp
  then show ?case
    using honest_induct.IH honest_axioms mining_def nCheck_add_h
    by (metis honest.h_height_add_h)
next
  case (dishonest_induct t)
  let ?c1 = "count True t"
  let ?c2 = "count False t"
  have "hd (\<lparr>Honest = False, State = dadd (State (hd t))\<rparr> # t) = \<lparr>Honest = False, State = dadd (State (hd t))\<rparr>" by simp
  then show ?case
    using dishonest_induct.IH mining.nCheck_add_m mining_axioms 
    by (smt (verit) bounded_dishonest_mining dishonest_induct.hyps(2) 
        dishonest_induct.hyps(3) le_add_diff_inverse2 n_check_weaken_depth_Node 
        n_check_weaken_distance_Node)
qed
end