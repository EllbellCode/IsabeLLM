...
next
  ...
  next
    ...
    proof (cases "x = t'" )
      ...
      with Cons
      have ***: "foldr max (map nHeight xs) 0 = nHeight tmax" by simp
      show ?thesis
    ...
    next
      ...
      moreover from False
      have *: "nHeight x \<le> nHeight t'" using Cons(3) by simp
      ultimately
      have "foldr max (map nHeight xs) 0 = nHeight t'" using Cons by auto
...
lemma branch_height:
 assumes "p \<in> set (nPaths t)"
  ...
  using assmsproof (induction t arbitrary: p)
  case (nNode x ts)
  then show
  ?case
  proof (cases ts)
    case Nil
    then have "nPaths (nNode x []) = [[x]]" by simp
    then have "p = [x]" using nNode.prems local.Nil by auto (*SAFE*)
    then have "length p = 1" by simp
    moreover
    have "nHeight (nNode x []) = 1" by simp
    ultimately
    show ?thesis by (simp add: local.Nil) (*SAFE*)
  next
  case (Cons t ts')
    then show ?thesis
    proof -
      have "p \<in> set (map (\<lambda>p. x # p) (concat (map nPaths (t # ts'))))"
        using nNode.prems by (simp add: local.Cons) (*SAFE*)
        then obtain p' where "p = x # p'" and "p' \<in> set (concat (map nPaths (t # ts')))" by fastforce (*SAFE*)
      then obtain u where "u \<in> set (t # ts')" and "p' \<in> set (nPaths u)"
        using nNode.prems by (auto simp: )
      then have "nHeight u \<ge> length p'" using local.Cons nNode.IH by blast (*SAFE*)
      have "foldr max (map nHeight (t # ts')) 0 \<ge> nHeight u"
        using \<open>u \<in> set (t # ts')\<close> by (rule subtree_height)
      then have "foldr max (map nHeight (t # ts')) 0 \<ge> length p'"
        using \<open>nHeight u \<ge> length p'\<close> by linarith
      have "nHeight (nNode x (t # ts')) = Suc (foldr max (map nHeight (t # ts')) 0)"
        by simp
      also have "... \<ge> Suc (length p')" using \<open>foldr max (map nHeight (t # ts')) 0 \<ge> length p'\<close> by simp
      also have "... = length (x # p')" by simp
      finally show ?thesis using \<open>p = x # p'\<close> by simp
    qed
  qed
qed
end